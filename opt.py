...
def get_opt(model):
    ...
    model = OPTForCausalLM.from_pretrained(model, torch_dtype=torch.float)
...
@torch.no_grad()
def opt_sequential(model, inps, attn_mask, opt_bounds, dev):
    print('Starting ...')
...
if __name__ == '__main__':
    ...
    if args.wbits < 16 and not args.nearest:
        tick = time.time()
        inps, attn_mask = get_inps(model, dataloader, DEV)

        if args.abtype=="optim":
            opt_bounds = get_optbounds(model,inps,attn_mask,args.wbits,DEV)
        
        elif args.abtype=="pso":
            opt_bounds = get_psobounds(model,inps,attn_mask,args.wbits,DEV)

        elif args.abtype=="gdiv":
            opt_bounds = get_gdivbounds(model,inps,attn_mask,args.wbits,DEV)
        ...
        quantizers = opt_sequential(model, inps, attn_mask, opt_bounds, DEV)
