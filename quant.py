...
def quantize_dequantize(x, scale, zero, maxq, eps=1e-9):
    q = torch.clamp(round_ste(x / scale.clamp_min(eps) + zero), 0, maxq)
    return scale * (q - zero)
...
class Quantizer(nn.Module):
    ...
    def configure(
        ...
        a_bound=torch.tensor(1.0),
        b_bound=torch.tensor(1.0),
        trits=False
    ):
        ...
        self.a_bound = a_bound   #bound is a [n,1] matrix
    ...
    def find_params(self, x, weight=False):
        ...
        self.a_bound = self.a_bound.to(dev)
        self.b_bound = self.b_bound.to(dev) 
        xmin = torch.minimum(x.min(1)[0], tmp) * self.b_bound.squeeze(-1)
        xmax = torch.maximum(x.max(1)[0], tmp) * self.a_bound.squeeze(-1)
        ...
        if not self.perchannel:   # channel means the dimension over the row,col 2-dimensions
            if weight:
                tmp = shape[0]
            else:
                tmp = shape[1] if len(shape) != 3 else shape[2]
            self.scale = self.scale.repeat(tmp)   #a.repeat(c,m,n) means repeat a first in n's dimension for n times,then in m's dimension for m times and in c's dimension for c times.
